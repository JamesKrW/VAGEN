#!/bin/bash
# navigation - default_gae - concat - Qwen/Qwen2.5-VL-7B-Instruct
#
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-navigation_ppo_qwen25vl7b_base_common}
...
PYTHONUNBUFFERED=1 python3 -m vagen.training.main \
    ...
    "${BASE[@]}" \
    data.train_files="$SCRIPTDIR/train_navigation_base_common.yaml" \
    data.val_files="$SCRIPTDIR/val_navigation_base_common.yaml" \
