#!/bin/bash
# frozenlake - grpo - concat - Qwen/Qwen2.5-VL-3B-Instruct
#
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-frozenlake_grpo_qwen25vl3b_nofilter_text}
...
PYTHONUNBUFFERED=1 python3 -m vagen.training.main \
    ...
    "${BASE[@]}" \
    data.train_files="$SCRIPTDIR/train_frozenlake_text.yaml" \
    data.val_files="$SCRIPTDIR/val_frozenlake_text.yaml" \
